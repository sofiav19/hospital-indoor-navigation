...
const NAV_ITEMS = [
  { key: "home", label: "Inicio", route: "/", icon: "home-outline" as const },
  { key: "map", label: "Mapa", route: "/navigate", icon: "map-outline" as const },
  { key: "settings", label: "Ajustes", route: "/settings", icon: "settings-outline" as const },
];

function isItemActive(pathname: string, itemKey: string) {
  if (itemKey === "home") return pathname === "/";
  // Search and AR are still part of the main map flow, so they keep the map tab active.
    ...
    <View style={[styles.wrap, { paddingBottom: Math.max(insets.bottom, 10) }]}>
      <View style={styles.bar}>
        {NAV_ITEMS.map((item) => {
          ...
          // Reopen the mode the user was using last
          const targetRoute =
...
const styles = StyleSheet.create({
  wrap: {backgroundColor: "#ffffff", borderTopWidth: 1, borderTopColor: "#d9e0e5", },
  bar: {
    flexDirection: "row",
